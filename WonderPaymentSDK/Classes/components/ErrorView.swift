class ErrorView : TGLinearLayout {
    ...
    lazy var errorColor = WonderPayment.uiConfig.errorColor
    lazy var errorCodeLabel = Label(errorMessage?.code ?? "",color: errorColor, size: 14, fontStyle: .medium)
    lazy var errorMsgLabel = Label(errorMessage?.message ?? "",color: errorColor, size: 14, fontStyle: .medium)
    ...
    private func initView() {
        ...
        let icon = UIImageView(image: "error".svg?.withTintColor(errorColor))
        icon.tg_centerX.equal(0)
        addSubview(icon)
        
        let titleLabel = Label("paymentFailed".i18n,color: errorColor, size: 18, fontStyle: .medium)
